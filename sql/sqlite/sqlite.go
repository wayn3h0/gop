...
func (r *rows) Scan(dest ...interface{}) error {
	...
	if err != nil {
		return errors.Wrap(err, "sqlite: could not parse columns in current row")
	}
...
func (t *transaction) Execute(statement string, args ...interface{}) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrapf(err, "sqlite: could not execute sql statement %q", statement)
	}

	n, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "sqlite: could not get number of rows affected")
	}
...
func (t *transaction) Query(statement string, args ...interface{}) (sql2.Rows, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "sqlite: could not query rows %q", statement)
	}
...
func (t *transaction) Rollback() error {
	err := t.Tx.Rollback()
	if err != nil {
		return errors.Wrap(err, "sqlite: could not abort the transaction")
	}
...
func (t *transaction) Commit() error {
	err := t.Tx.Commit()
	if err != nil {
		return errors.Wrap(err, "sqlite: could not commit the transaction")
	}
...
func (d *database) Begin() (sql2.Transaction, error) {
	tx, err := d.DB.Begin()
	if err != nil {
		return nil, errors.Wrap(err, "sqlite: could not start a transaction")
	}
...
func NewDatabase(dataSource string) (sql2.Database, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "sqlite: could not open the database %q", dataSource)
	}
