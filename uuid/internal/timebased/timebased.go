...
func NewUUID() ([]byte, error) {
	...
	if !now.After(lastGenerated) {
		...
		_, err := rand.Read(b)
		if err != nil {
			return nil, errors.Wrap(err, "uuid: could not generate clock sequence")
		}
	...
	if nodeID == nil {
		...
		if err != nil {
			return nil, errors.Wrap(err, "uuid: could not get network interfaces")
		}
		...
		if nodeID == nil {
			...
			if err != nil {
				return nil, errors.Wrap(err, "uuid: could not generate node id")
			}
		}
	}
